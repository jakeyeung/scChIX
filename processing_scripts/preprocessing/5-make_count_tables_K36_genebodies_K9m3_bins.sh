...
bedfile="/hpc/hub_oudenaarden/jyeung/data/dblchic/gastrulation/from_analysis/coords_filtered/coords_K36_genebodies_K9m3_bins.2021-07-10.bed"
[[ ! -e $bedfile ]] && echo "$bedfile not found, exiting" && exit 1

jsuffix="topfeatures_K36_genebodies_K9m3_bins"
outdir="${inmain}/counts_tables_${jsuffix}"
[[ ! -d $outdir ]] && mkdir $outdir

for inbam in `ls -d $inmain/*.bam`; do
    bname=$(basename $inbam)
    bname=${bname}
    outftab=${outdir}/${bname}.count_table_${jsuffix}.txt
