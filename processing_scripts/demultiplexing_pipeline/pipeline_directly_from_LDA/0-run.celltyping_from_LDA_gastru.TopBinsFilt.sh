...
for jquant in $jquantvec; do
  ...
  for jmark2 in $jmark2vec; do
    ...
    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -ldainputvec $inldavec -marks $markvec -pseudobulk_reference ${pbulkref} -keeptop 150 -gene2tss_reference $tssref -outprefix $outprefix"
