...
# File: ~/projects/scChIX/analysis_scripts/H3K9me3-H3K36me3_gastrulation_analysis/7-analyze_coords_dbl_explore_logL_matrix.R
# description
rm(list=ls())
...
inf.output <- "/Users/yeung/Dropbox/macbookpro_data/data/dblchic/gastrulation/snakemake_downstream_outputs/scchix_inputs_clstr_by_celltype_K36-K9m3.RData"
...
# saveRDS(clst2celltype, "/Users/yeung/data/dblchic/gastrulation/metadata/clst2celltype_H3K36me3.rds")
# saveRDS(clst2k9, "/Users/yeung/data/dblchic/gastrulation/metadata/clst2cluster_H3K9m3.rds")
...
set.seed(0)
# jcells <- names(fits.out); names(jcells) <- jcells
jclst <- "Erythroid"
jcells <- subset(dat.meta, cluster == jclst)$cell
# jcell <- "E10p5-B6C-K36-K9m3-190409-1_254"
(jcell <- sample(x = jcells, size = 1, replace = FALSE))
LL.mat <- fits.out[[jcell]]$ll.mat
LL.mat.tmp <- data.frame(K36 = rownames(LL.mat), LL.mat, stringsAsFactors = FALSE)
LL.long.tmp <- data.table::melt(LL.mat.tmp, id.vars = "K36", value.name = "LL", variable.name = "K9") %>%
  mutate(K36 = as.character(K36), 
         K9 = as.character(K9)) %>%
  rowwise() %>%
  mutate(K36.annot = AssignHash(K36, clst2celltype, null.fill = K36),
         K9.annot = AssignHash(K9, clst2k9, null.fill = K9))


m <- ggplot(LL.long.tmp, aes(x = K36.annot, y = K9.annot, fill = LL)) +
  geom_tile() +
  theme_bw(18) +
  scale_fill_viridis_c() +
  ggtitle("", paste(jclst, jcell)) +
  theme(aspect.ratio= 3/9, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
