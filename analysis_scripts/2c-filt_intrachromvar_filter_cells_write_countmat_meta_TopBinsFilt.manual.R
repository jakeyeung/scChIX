...
infs.meta <- lapply(jmarks, function(jmark){
  inf.meta <- file.path(inmain, paste0("metadata_var_filt.", jmark, ".", jdate.output, ".rds"))
...
infs.countmat <- lapply(jmarks, function(jmark){
  inf.countmat <- file.path(inmain.counts, paste0("countmat_featurefilt.", jmark, ".", jdate, ".rds"))
...
for (jmark in jmarks){
  ...
  outf.countmat.tmp <- file.path(outdir, paste0("countmat_var_filt_TopBinsFilt.", jmark, ".", Sys.Date(), ".rds"))
