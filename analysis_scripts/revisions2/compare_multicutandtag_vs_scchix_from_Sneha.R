...
# File: ~/projects/scChIX/analysis_scripts/revisions2/compare_multicutandtag_vs_scchix_from_Sneha.R
# 
...
indir.mct <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChIX_revisions/Gopalan_et_al/from_Sneha"
...
infs.mct <- lapply(marks.mct, function(mark){
  inf.tmp <- file.path(indir.mct, paste0(mark, ".txt"))
  return(inf.tmp)
})
...
dats.mct <- lapply(marks.mct, function(jmark){
  ...
  fread(jinf, header = FALSE) %>%
    ...
    dplyr::rename("barcode" = "V1",
                  "fragments_in_peaks" = "V2",
                  "fragments_per_cell" = "V3") 
}) %>%
  bind_rows()
...
dats.mct.filt <- dats.mct %>% filter(fragments_per_cell > 0) %>%
  dplyr::select(barcode, fragments_per_cell, dataset, mark)
