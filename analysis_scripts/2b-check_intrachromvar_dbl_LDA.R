...
# File: ~/projects/scChIX/analysis_scripts/2b-check_intrachromvar_dbl_LDA.R
#
...
jcutoff.lst <- lapply(jmarks, function(jmark){
  jcutoff <- quantile(dat.umap.merge.lst[[jmark]]$cell.var.within.sum.norm, 0.15)
})
...
outmain <- "/home/jyeung/hub_oudenaarden/jyeung/data/dblchic/gastrulation/from_analysis/rds_objs_celltyping_from_less_stringent2/var_filtered"
outdir <- file.path(outmain, jstr)
dir.create(outdir)

outpdf <- file.path(outdir, paste0("plots_check_intrachrom_var.", jstr, ".", Sys.Date(), ".pdf"))
...
for (jmark in jmarks){
  ...
  outf.var.tmp <- file.path(outdir, paste0("intrachrom_var_outputs.", jmark, ".", Sys.Date(), ".rds"))


  # write filtered meta data
  dat.meta.tmp <- subset(dat.var.tmp, !is.bad)
  outf.meta.tmp <- file.path(outdir, paste0("celltyping_var_filt.", jmark, ".", Sys.Date(), ".rds"))

  # write filtered count mat
  cells.keep.tmp <- dat.meta.tmp$cell
  dat.countmat.tmp <- countmats.lst[[jmark]][, cells.keep.tmp]
  outf.countmat.tmp <- file.path(outdir, paste0("countmat_var_filt.", jmark, ".", Sys.Date(), ".rds"))
