...
# File: ~/projects/scChIX/analysis_scripts/3d-recluster_from_cleaned_cells_make_objects_LessStringent2.TopBinsFilt.R
#
...
jappend <- paste0("TopBinsFilt")

jprefix <- "filtered_count_tables_filtered_counts_varcutoffmin_0.03.noLSIfilt"
jsuffix <- paste0(jprefix, "_", jappend, "_", jstr)

jmain <- file.path(hubprefix, paste0("jyeung/data/dblchic/gastrulation/from_analysis/rds_objs_celltyping_from_less_stringent2_", jappend))
dir.create(jmain, recursive = FALSE)
outmain <- file.path(jmain, jmarks[[2]])
dir.create(outmain, recursive = FALSE)

jname <- "countmat_featurefilt"
...
for (jmark in jmarks){
  ...
  outdir <- file.path(outmain, paste(jprefix, jappend, sep = "_"))
  dir.create(outdir)
