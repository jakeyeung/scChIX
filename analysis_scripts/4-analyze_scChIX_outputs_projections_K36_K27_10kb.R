...
# File: ~/projects/scChIX/analysis_scripts/4-analyze_scChIX_outputs_projections_K36_K27.R
#
...
jprefix <- paste0("from_pipeline_binsize_", jbinsize)
# jsuffix <- "filtered_count_tables_filtered_counts_varcutoffmin_0.03.noLSIfilt"
jsuffix <- paste0("var_filtered_manual2nocenternoE8unifyK36_", jstr)

outmain <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/dblchic/gastrulation/from_analysis/scchix_downstream_plots_binsize_", jbinsize)
dir.create(outmain)
...
# metamain <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/dblchic/gastrulation/from_analysis/rds_objs_celltyping_from_less_stringent2/binsize_", jbinsize)
metamain <- "/home/jyeung/hub_oudenaarden/jyeung/data/dblchic/gastrulation/from_analysis/rds_objs_celltyping_from_less_stringent2/var_filtered_manual2nocenternoE8unifyK36/K36_K27_K36-K27"
...
dat.meta.lst <- lapply(jmarks, function(jmark){
  ...
  infmeta <- file.path(metamain, paste0("meta_var_filt.", jmark, ".2021-07-29.rds"))
  print(infmeta)
...
infrdata <- file.path(hubprefix, paste0("jyeung/data/dblchic/gastrulation/scchix_pipeline_from_LDA_binsize_", jbinsize, "/scchix_outputs_objs/", jsuffix, "/unmix_scchix_inputs_clstr_by_celltype_", jmarkdbl, ".removeNA_FALSE.RData"))
print(infrdata)
...
# Select second highest to see if cluster 6 pops out  ---------------------
...
coords.dbl.second <- lapply(cell.vec, function(jcell){
  jfit <- fits.out[[jcell]]
  ...
  jval.i <- kit::topn(as.matrix(jfit$ll.mat), n = jtop, decreasing = TRUE)[jtop]
  jcoord <- which(as.matrix(jfit$ll.mat) == as.matrix(jfit$ll.mat)[jval.i], arr.ind = TRUE)
  jmax <- as.matrix(p.mat)[jval.i]
...
m.grid.second <- ggplot(coords.dbl.second, aes(x = louv.act, y = louv.repress, color = w)) +
