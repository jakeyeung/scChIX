...
outdir1 <- file.path(inmain, "dbl_k36_cleaned")
dir.create(outdir1)
outdir2 <- file.path(inmain, "dbl_k36_k9m3_cleaned")
dir.create(outdir2)

# write new umaps

dat.umap.filt.lst1 <- list(`K36` = dat.umap.lst$`K36` %>% filter(cluster != bad.clst.k36),
...
# clean up dbl, K36, and k9

dat.umap.filt.lst2 <- list(`K36` = dat.umap.lst$`K36` %>% filter(cluster != bad.clst.k36),
                          `K9m3` = dat.umap.lst$K9m3 %>% filter(cluster != bad.clst.k9),
                          `K36-K9m3` = dat.umap.lst$`K36-K9m3` %>% filter(plate != bad.plate.dbl))
cells.keep2 <- unlist(lapply(dat.umap.filt.lst2, function(x) x$cell))

count.mat.lst2 <- lapply(count.mat.lst, function(jmat){
  cols.keep.tmp <- colnames(jmat) %in% cells.keep2
  jmat[, cols.keep.tmp]
})



jdat.out2 <- lapply(jmarks, function(jmark){
  outtmp.name <- paste0("celltyping_output_filt.", jmark, ".", Sys.Date(), ".rds")
  jdat <- dat.umap.filt.lst2[[jmark]]
  assertthat::assert_that(nrow(jdat) > 0)
  assertthat::assert_that(ncol(jdat) > 0)
  print(dim(jdat))
  saveRDS(jdat, file = file.path(outdir2, outtmp.name))
  return(jdat)
})


jmat.out2 <- lapply(jmarks, function(jmark){
  outtmp.name <- paste0("countmat_output_filt.", jmark, ".", Sys.Date(), ".rds")
  jmat <- count.mat.lst2[[jmark]]
  assertthat::assert_that(nrow(jmat) > 0)
  assertthat::assert_that(ncol(jmat) > 0)
  print(dim(jmat))
  saveRDS(jmat, file = file.path(outdir2, outtmp.name))
  return(jmat)
})
...
print(lapply(count.mat.lst, dim))
print(lapply(count.mat.lst1, dim))
print(lapply(count.mat.lst2, dim))
