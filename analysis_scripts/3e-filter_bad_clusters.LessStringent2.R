...
# File: ~/projects/scChIX/analysis_scripts/3e-filter_bad_clusters.LessStringent2.R
#
...
outmain <- file.path(inbase, "clusters_filtered")
dir.create(outmain)
...
infs.meta <- lapply(jmarks, function(jmark){
  inf.meta.tmp <- file.path(inmain, paste0("celltyping_output_filt.", jmark, ".2021-07-14.rds"))
...
dats.meta <- lapply(infs.meta, readRDS)
