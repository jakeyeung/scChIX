...
[[ ! -e $infdblinput ]] && echo "$infdblinput infdblinput not found, exiting" && exit 1
[[ ! -e $infdbloutput ]] && echo "$infdbloutput infdbloutput not found, exiting" && exit 1

# [[ ! -e $inflda1 ]] && echo "$inflda1 inflda1 not found, exiting" && exit 1
# [[ ! -e $inflda2 ]] && echo "$inflda2 inflda2 not found, exiting" && exit 1
[[ ! -e $infldadbl ]] && echo "$infldadbl infldadbl not found, exiting" && exit 1

[[ ! -e $infmat1 ]] && echo "$infmat1 infmat1 not found, exiting" && exit 1
[[ ! -e $infmat2 ]] && echo "$infmat2 infmat2 not found, exiting" && exit 1
# [[ ! -e $infmatdbl ]] && echo "$infmatdbl infmatdbl not found, exiting" && exit 1


. /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -mark1 $mark1 -mark2 $mark2 -inf_dbl_input $infdblinput -inf_dbl_output $infdbloutput -inf_mark1 $infmat1 -inf_mark2 $infmat2 -inf_dblmark_lda $infldadbl -outprefix $outprefix
